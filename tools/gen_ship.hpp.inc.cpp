...
int main() {
    const auto src_fname = std::string_view{"./assets/api_mst_ship.json"};
    const auto dst_fname = std::filesystem::path{"./library/core/constants/ship.hpp.inc"};
    ...
    if (not std::ranges::is_sorted(data, {}, &kcv::tools::api_mst_ship::value_type::api_id)) {
        std::println(stderr, "error: api_mst_ship.json is not sorted.");
        return EXIT_FAILURE;
    }
    ...
    for (const auto& [api_id, api_name] : data) {
        ofs << "    {kcv::kcsapi::ship_id{" << api_id << "}, \"" << api_name << "\"},\n";
    }
    ofs << "};";
}