...
jq -r '.[] | "{\(.api_id), \"\(.api_name)\", \(.api_stype)},"'   ./assets/api_mst_ship/api_mst_ship.json          > ./include/constants/ship.hpp.inc
jq -r '.[] | "{\(.api_id), \"\(.api_name)\", \(.api_type[3])},"' ./assets/api_mst_slotitem/api_mst_slotitem.json  > ./include/constants/equipment.hpp.inc