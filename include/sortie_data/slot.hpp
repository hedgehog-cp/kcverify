#ifndef KCVERIFY_SORTIE_DATA_SLOT_HPP_INCLUDED
#define KCVERIFY_SORTIE_DATA_SLOT_HPP_INCLUDED

// std
#include <cstdint>
#include <optional>

// kcv
#include "eoen/database/sortie/sortie_equipment_slot.hpp"
#include "sortie_data/equipment.hpp"

namespace kcv {

/// @brief スロット.
class slot final {
   public:
    using eoen_type = kcv::eoen::database::sortie::sortie_equipment_slot;

    static constexpr auto from_eoen(const eoen_type& src, const kcv::kcsapi::api_mst_slotitem& mst) -> slot {
        return slot{
            src.aircraft_current,
            src.aircraft_max,
            src.equipment.transform([&](const auto& e) { return kcv::equipment::from_eoen(e, mst); }),
        };
    }

    constexpr slot(std::int32_t current, std::int32_t max, std::optional<kcv::equipment> equipment) noexcept
        : aircraft_current_{std::move(current)}
        , aircraft_max_{std::move(max)}
        , equipment_{std::move(equipment)} {}

    constexpr auto aircraft_current() const noexcept -> decltype(auto) {
        return this->aircraft_current_;
    }

    constexpr auto aircraft_max() const noexcept -> decltype(auto) {
        return this->aircraft_max_;
    }

    constexpr auto equipment() const noexcept -> decltype(auto) {
        return this->equipment_;
    }

   private:
    /// @brief 現在搭載数.
    std::int32_t aircraft_current_;

    /// @brief 最大搭載数.
    std::int32_t aircraft_max_;

    /// @brief 装備.
    std::optional<kcv::equipment> equipment_;
};

}  // namespace kcv

#endif  // KCVERIFY_SORTIE_DATA_SLOT_HPP_INCLUDED
