#ifndef KCVERIFY_MODELS_KCSAPI_API_REQ_COMBINED_BATTLE_EACH_BATTLE_WATER_RESPONSE_HPP_INCLUDED
#define KCVERIFY_MODELS_KCSAPI_API_REQ_COMBINED_BATTLE_EACH_BATTLE_WATER_RESPONSE_HPP_INCLUDED
...
namespace each_battle_water {

/// @brief api_req_combined_battle/each_battle_water.json.
...
struct response final {
    ...
    /// @brief 第一次砲撃戦 味方主力 vs 敵主力. api_hourai_flag[0]==0のときnull. メンバの配列長はすべて攻撃フェイズの回数.
    std::optional<kcv::kcsapi::api_hougeki1> api_hougeki1;

    /// @brief 第二次砲撃戦 味方主力→敵全体 vs 敵主力→味方全体. api_hourai_flag[1]==0のときnull. メンバの配列長はすべて攻撃フェイズの回数.
    std::optional<kcv::kcsapi::api_hougeki1> api_hougeki2;

    /// @brief 第三次砲撃戦 味方随伴 vs 敵随伴. api_hourai_flag[2]==0のときnull. メンバの配列長はすべて攻撃フェイズの回数.
...
#endif  // KCVERIFY_MODELS_KCSAPI_API_REQ_COMBINED_BATTLE_EACH_BATTLE_WATER_RESPONSE_HPP_INCLUDED
