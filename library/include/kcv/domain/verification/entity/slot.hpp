#ifndef KCV_DOMAIN_VERIFICATION_ENTITY_SLOT_HPP_INCLUDED
#define KCV_DOMAIN_VERIFICATION_ENTITY_SLOT_HPP_INCLUDED

// std
#include <cstdint>
#include <optional>

// kcv
#include "kcv/domain/verification/entity/equipment.hpp"

namespace kcv {

/// @brief スロット.
class slot final {
   public:
    slot(std::int32_t current, std::int32_t max, std::optional<kcv::equipment> equipment) noexcept
        : aircraft_current_{current}
        , aircraft_max_{max}
        , equipment_{std::move(equipment)} {}

    auto aircraft_current() const noexcept -> std::int32_t {
        return this->aircraft_current_;
    }

    auto aircraft_max() const noexcept -> std::int32_t {
        return this->aircraft_max_;
    }

    auto equipment() const noexcept -> const std::optional<kcv::equipment>& {
        return this->equipment_;
    }

   private:
    /// @brief 現在搭載数.
    std::int32_t aircraft_current_;

    /// @brief 最大搭載数.
    std::int32_t aircraft_max_;

    /// @brief 装備.
    std::optional<kcv::equipment> equipment_;
};

}  // namespace kcv

#endif  // KCV_DOMAIN_VERIFICATION_ENTITY_SLOT_HPP_INCLUDED
